...
This is a single-angler notebook — not a social network. A catch photo fills the species field from the saltwater catalog; tap the name only if it is wrong.
...
| `OPENAI_API_KEY` | Photo species ID, restricted to the saltwater catalog (inshore / offshore). | Demo fill from the saltwater catalog. Never freshwater names. |
| `OPENAI_VISION_MODEL` | Vision model for photo species ID. | `gpt-4o` |
