...
This is a single-angler notebook — not a social network. You pick the saltwater species yourself — tap a chip or type a name.
...
| `OPENAI_API_KEY` | Optional fish-photo detector for **Find fishing photos in your library**. Species on Log and Backfill is picked by hand. | Demo detector from the filename / a hash. |
| `OPENAI_VISION_MODEL` | Vision model for the library fish-photo detector. | `gpt-4o` |
| `WORLDTIDES_API_KEY` | Optional worldwide tide extremes when no NOAA station is nearby ([WorldTides](https://www.worldtides.info/)) | Catch weather and Plan use the nearest free [NOAA CO-OPS](https://tidesandcurrents.noaa.gov/) station on US coasts (no key; MLLW, GMT). No invented ocean tides on freshwater or inland pins. Plan uses a labeled demo series only when NOAA and this key are both unavailable. |
...
Shipping notes: [docs/SHIPPING.md](docs/SHIPPING.md). App Store / Capacitor wrap: [docs/app-store.md](docs/app-store.md). Public privacy policy: `/privacy`.
