...
- **Calendar Log** — primary bottom tab (`/calendar`; `/history` redirects here). Opens on the **month calendar** (day cells show stacked catch photos and +N). Tap a day for a **satellite map** of that day’s catch pins (one pin per spot; trips with no GPS stay in the list only) plus the day’s trips in time order. Switch to **List** or **Grid** for the old History browse modes. Filter by inshore / offshore, time of day (derived from the catch clock), moon phase, wind direction, and pressure trend. Presets like “Cloudy 70–80°F” and “Full moon”. No season filters. CSV export.
