...
- **Calendar Log** — primary bottom tab (`/calendar`; `/history` redirects here). Opens on the **month calendar** (day cells show stacked catch photos and +N; a copper year badge marks dates that have trips in other years). Tap a day for **that calendar date across years** (e.g. Sep 2 in 2023, 2024, and 2025 together), year labels on each trip, and a **satellite map** of every pin from those dates. Toggle **This year only** if you want just the month you are browsing (All years is the default). Share or unshare applies to the selected year’s day. Switch to **List** or **Grid** for the old History browse modes. Filter by inshore / offshore, time of day (derived from the catch clock), moon phase, wind direction, and pressure trend. Presets like “Cloudy 70–80°F” and “Full moon”. No season filters. CSV export.
