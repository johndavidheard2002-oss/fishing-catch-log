...
- **Calendar Log** — primary bottom tab (`/calendar`; `/history` redirects here). Opens on the **month calendar** (day cells show stacked catch photos and +N). Tap a day for that day’s trips in time order, each with its own spot — two pins in one day stay two pins. Switch to **List** or **Grid** for the old History browse modes. Filter by inshore / offshore, time of day (derived from the catch clock), moon phase, wind direction, and pressure trend. Presets like “Cloudy 70–80°F” and “Full moon”. No season filters. CSV export.
