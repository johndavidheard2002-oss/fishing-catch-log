...
- **Find fishing photos in your library** — import old trips from this device. Choose photos (or a folder on a computer). Skip shots that do not look like fish, then **Add to log?** for each candidate. Photo time comes from EXIF and is editable. Yes opens the log form; after save, History opens on that day. Remaining photos stay queued. Nothing is added until you confirm. Only your selected photos are processed. The browser cannot silently scan the whole camera roll.
