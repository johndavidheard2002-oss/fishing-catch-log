...
import { heightAndDirectionAt } from "./tides/snapshot";
...
describe("planned day and photo tide labels", () => {
  ...
  it("shows the plan-day clock when height matches a catch just after High, not a distant Low", () => {
    ...
    expect(label).not.toMatch(/dropping/i);
  ...
  it("picks matching incoming/outgoing and never the opposite equal-height time", () => {
    ...
    expect(outgoing).not.toMatch(/dropping/i);
    expect(
      plannedSpotSameTide(
  ...
  it("on a High AM / Low PM day prefers afternoon outgoing over 11:08 PM incoming", () => {
    ...
    const height = heightAndDirectionAt(inglesideSep19.extremes, new Date("2026-09-20T04:08:00.000Z"))
      ?.heightFt;
    ...
    const pin = {
      ...
      longitude: -97.211,
      habitat: "saltwater-inshore" as const,
      caughtAt: "2026-08-02T04:08:00.000Z", // 11:08 PM CDT
      tideHeightFt: height!,
      tide: "outgoing",
    };
    ...
    const flippedByCatchSnap = plannedSpotSameTide(
      inglesideSep19,
      "2026-09-19",
      pin,
      zone,
    ...
    expect(flippedByCatchSnap).toBe(label);

    const onlyOppositeHeight = plannedSpotSameTide(
      {
        ...inglesideSep19,
        extremes: [
          { at: "2026-09-19T12:39:00.000Z", type: "high", heightFt: 0.8 },
          { at: "2026-09-20T01:13:00.000Z", type: "low", heightFt: 0.2 },
          { at: "2026-09-20T13:00:00.000Z", type: "high", heightFt: 1.0 },
        ],
      },
      "2026-09-19",
      { ...pin, tideHeightFt: 0.9, tide: "outgoing" },
      zone,
    );
    ...
    const spots = [{ id: "n-mud", placeName: mud.placeName, sourceCatchId: mud.id }];
    const catchSnapIncoming: TideSnapshot = {
      applies: true,
      tide: "incoming",
      heightFt: height!,
      nextHighAt: null,
    ...
    expect(applyCatchTideSnapshot(pin, catchSnapIncoming).tide).toBe("outgoing");
    const mudPin = pinForPlannedSpot(spots[0], { catches: [mud] });
    ...
    expect(chips["n-mud"]).not.toMatch(/dropping/i);
    expect(chips["n-mud"]).not.toBe("11:08 PM incoming");
  });

  it("chips a falling afternoon on a High AM / Low PM day as outgoing, never incoming", () => {
    const inglesideSep19: TideSnapshot = {
      applies: true,
      tide: "outgoing",
      heightFt: 0.5,
    ...
    )["n-aft"]).toMatch(/PM outgoing/);
  });

  it("matches catch height + flood/ebb on Enbridge Sep 19 High AM / Low PM, never the opposite limb", () => {
    // John / Tim's trip: High 7:39 AM 0.8 ft · Low 8:13 PM 0.2 ft · Enbridge, Ingleside.
    const inglesideSep19: TideSnapshot = {
      applies: true,
      tide: "outgoing",
      heightFt: 0.5,
    ...
    const zone = "America/Chicago";
    const afternoon = new Date("2026-09-19T21:02:00.000Z"); // 4:02 PM CDT High→Low
    const afterLow = new Date("2026-09-20T04:08:00.000Z"); // 11:08 PM CDT after Low
    const beforeHigh = new Date("2026-09-19T09:21:00.000Z"); // 4:21 AM CDT
    const afternoonHeight = heightAndDirectionAt(inglesideSep19.extremes, afternoon)!.heightFt;
    const afterLowHeight = heightAndDirectionAt(inglesideSep19.extremes, afterLow)!.heightFt;
    const beforeHighHeight = heightAndDirectionAt(inglesideSep19.extremes, beforeHigh)!.heightFt;
    const catchDayOutgoing: TideSnapshot = {
      applies: true,
      tide: "low",
      heightFt: afternoonHeight,
      nextHighAt: null,
      nextHighFt: null,
      nextLowAt: null,
      nextLowFt: null,
    ...
    const afternoonPin = {
      ...
      tideHeightFt: afternoonHeight,
      tide: "outgoing" as const,
    };
    const afternoonOutgoing = plannedSpotSameTide(
      inglesideSep19,
      "2026-09-19",
      afternoonPin,
      zone,
    );
    expect(afternoonOutgoing).toMatch(/PM outgoing/);
    expect(afternoonOutgoing).not.toMatch(/incoming|dropping/i);
    expect(afternoonOutgoing).not.toBe("11:08 PM incoming");

    const afterLowIncoming = plannedSpotSameTide(
      inglesideSep19,
      "2026-09-19",
      {
        latitude: 27.877,
        longitude: -97.211,
        habitat: "saltwater-inshore",
        caughtAt: "2026-08-15T04:08:00.000Z",
        tideHeightFt: afterLowHeight,
        tide: "incoming",
      },
      zone,
    );
    expect(afterLowIncoming).toMatch(/incoming/i);
    expect(afterLowIncoming).not.toMatch(/outgoing|dropping/i);

    const beforeHighIncoming = plannedSpotSameTide(
      inglesideSep19,
      "2026-09-19",
      {
        latitude: 27.877,
        longitude: -97.211,
        habitat: "saltwater-inshore",
        caughtAt: "2026-08-15T09:21:00.000Z",
        tideHeightFt: beforeHighHeight,
        tide: "incoming",
      },
      zone,
    );
    expect(beforeHighIncoming).toMatch(/AM incoming/);
    expect(beforeHighIncoming).not.toMatch(/outgoing|dropping/i);

    expect(catchLoggedOrSampledHeight(afternoonPin, catchDayOutgoing)).toBe(afternoonHeight);
    expect(catchFloodEbbDirection(afternoonPin, catchDayOutgoing, inglesideSep19.extremes, afternoon, zone)).toBe(
      "falling",
    );
    const heightMatchOutgoing = plannedSpotSameTide(
      inglesideSep19,
      "2026-09-19",
      {
        ...afternoonPin,
    ...
    expect(heightMatchOutgoing).toMatch(/PM outgoing/);
    expect(heightMatchOutgoing).not.toBe("11:08 PM incoming");
    expect(heightMatchOutgoing).not.toMatch(/incoming/i);

    const storedLow = plannedSpotSameTide(
      inglesideSep19,
      "2026-09-19",
      { ...afternoonPin, tide: "low", tideHeightFt: afternoonHeight },
      zone,
      catchDayOutgoing,
    );
    expect(storedLow).toMatch(/PM outgoing/);
    expect(storedLow).not.toBe("11:08 PM incoming");
    expect(applyCatchTideSnapshot({ ...afternoonPin, tide: "low" }, catchDayOutgoing).tide).toBe(
      "outgoing",
    );
    ...
    const mud = catchOf({
      ...
      latitude: 27.877,
      longitude: -97.211,
      caughtAt: afternoonPin.caughtAt,
      tide: "low",
      tideHeightFt: afternoonHeight,
    });
    const corpus = catchOf({
      id: "c-corpus-height",
      placeName: "Corpus Christi, Texas",
      habitat: "saltwater-inshore",
      latitude: 27.8,
      longitude: -97.4,
      caughtAt: "2026-08-15T21:56:00.000Z",
      tide: "outgoing",
      tideHeightFt: heightAndDirectionAt(
        inglesideSep19.extremes,
        new Date("2026-09-19T21:56:00.000Z"),
      )!.heightFt,
    });
    const chips = sameTideChipsForSpots(
      [
        { id: "n-mud", placeName: mud.placeName, sourceCatchId: mud.id },
        { id: "n-corpus", placeName: corpus.placeName, sourceCatchId: corpus.id },
      ],
      inglesideSep19,
      "2026-09-19",
      { catches: [mud, corpus] },
    ...
    expect(chips["n-mud"]).toMatch(/PM outgoing/);
    expect(chips["n-mud"]).not.toBe("11:08 PM incoming");
    expect(chips["n-corpus"]).toMatch(/PM outgoing/);
    expect(chips["n-corpus"]).not.toMatch(/incoming/i);
  });
  ...
  it("chips Oct 11 Enbridge catches with interpolated incoming, not a leftover Low 2:42 PM", () => {
    ...
    expect(leftoverLow).toBeTruthy();
    expect(leftoverLow).not.toContain("2:42");
    expect(leftoverLow).not.toBe("Low 2:42 PM");
    const ship = catchOf({
      id: "c-ship-drum",
      placeName: "Ship channel",
      species: "Black Drum",
  ...
  it("never leaves a Planned row blank when that day has tides (regression after #90)", () => {
    const oct9: TideSnapshot = {
      applies: true,
      tide: "incoming",
      heightFt: 1.4,
    ...
    const catches = [1, 2, 3, 4, 5, 6].map((n) =>
      catchOf({
        id: `blank-c-${n}`,
        placeName: n % 2 === 0 ? "Port Aransas, Texas" : "Beach marker 42",
        habitat: "saltwater-inshore",
        latitude: n === 1 ? 27.84 : null,
        longitude: n === 1 ? -97.05 : null,
        caughtAt: n === 3 ? "" : `2026-08-0${Math.min(n, 9)}T00:00:00.000Z`,
        tide: n === 5 ? "outgoing" : null,
        tideHeightFt: n === 1 ? 4.5 : n === 5 ? 1.8 : null,
      }),
    );
    const spots = uniqueNotesByPlace(
      catches.map((row) => ({
    ...
    const noSnaps = sameTideChipsForSpots(spots, oct9, "2026-10-09", { catches });
    ...
    expect(catchNeedsPlanTideFetch(undefined)).toBe(true);
    expect(catchNeedsPlanTideFetch(catches[2])).toBe(true);
    expect(catchNeedsPlanTideFetch(catches[0])).toBe(true);
    expect(catchNeedsPlanTideFetch({ ...catches[0], tide: "outgoing" })).toBe(false);
    const withoutJournal = sameTideChipsForSpots(spots, oct9, "2026-10-09", {});
    expect(Object.keys(withoutJournal)).toHaveLength(spots.length);
    for (const spot of spots) {
      expect(withoutJournal[spot.id]).toBeTruthy();
    }
  });

  it("does not chip Oct 10 Low 2:42 PM onto an Oct 11 Enbridge plan, even before catchSnaps load", () => {
    const inglesideOct11: TideSnapshot = {
      applies: true,
      tide: "outgoing",
      heightFt: 0.7,
    ...
    const chips = sameTideChipsForSpots(spots, inglesideOct11, "2026-10-11", {
      catches: [ship, flounder],
    });
    const beforeJournal = sameTideChipsForSpots(spots, inglesideOct11, "2026-10-11", {});
    expect(chips["n-ship"]).toBeTruthy();
    expect(chips["n-demag"]).toBeTruthy();
    expect(chips["n-demag"]).not.toBe(chips["n-ship"]);
    expect(chips["n-demag"]).not.toContain("2:42");
    expect(chips["n-ship"]).not.toContain("2:42");
    expect(chips["n-demag"]).not.toMatch(/^Low\b/);
    expect(beforeJournal["n-demag"]).toBeTruthy();
    expect(beforeJournal["n-demag"]).not.toContain("2:42");
    expect(
      fallbackChipFromDayTides({ ...inglesideOct11, heightFt: null }, "2026-10-11", "America/Chicago"),
    ).not.toContain("2:42");
    expect(plannedSpotSameTide(inglesideOct11, "2026-10-11", null, "America/Chicago")).not.toContain(
      "2:42",
    );
  });
});
...
describe("catchRecordTideHeightLabel", () => {
  ...
  it("uses the logged catch height, including compact negative feet", () => {
    expect(catchRecordTideHeightLabel(record)).toBe("1.5");
    expect(catchRecordTideHeightLabel({ ...record, tideHeightFt: -0.5 })).toBe("-.5");
    expect(catchRecordTideHeightLabel({ ...record, tideHeightFt: 0.8 })).toBe("0.8");
  });

  it("still shows a logged height when the catch has no pin", () => {
    expect(
      catchRecordTideHeightLabel({
        ...record,
        latitude: null,
  ...
  it("does not replace a logged height with a NOAA snapshot heightFt", () => {
    ...
    expect(catchRecordTideHeightLabel(record, catchSnap)).toBe("1.5");
  });
  ...
  it("falls back to the NOAA sample at the catch clock when logged height is missing", () => {
    ...
    const sampled = heightAndDirectionAt(catchSnap.extremes, new Date(record.caughtAt));
    expect(sampled?.heightFt).toBeTruthy();
    expect(catchRecordTideHeightLabel({ ...record, tideHeightFt: null }, catchSnap)).toBe(
      formatCatchTideHeightFt(sampled!.heightFt),
    );
    expect(catchRecordTideHeightLabel({ ...record, tideHeightFt: null })).toBeNull();
    expect(catchRecordTideHeightLabel(null)).toBeNull();
  });
});
...
describe("Plan Planned panel wires day tides", () => {
  it("shows the planned day's tides and a closest tide on each photo row", () => {
    ...
    expect(plan).toContain("catchNeedsPlanTideFetch");
    expect(plan).not.toContain("note.sourceCatchId || note.sourceBaitId");
    ...
    expect(tides).toContain("catchNeedsPlanTideFetch");
    expect(tides).toContain("dayHighLowChip");
    expect(tides).toContain("sameDirectionMatches");
    ...
    expect(plan).not.toContain("setMapDay(picked.day)");
  });
});
